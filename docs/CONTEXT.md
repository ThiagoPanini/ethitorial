...
| **Source** | Referência a um item externo real (curso, livro, certificação) dentro de uma Section `with_sources`. Carrega metadata: nome, URL externa, autor/criador, descrição, imagem de capa (`cover`) e avatar do autor (`author_avatar`) — ambas as imagens opcionais e colocadas junto ao conteúdo. Pode existir sem nenhum `Post` vinculado. Pertence a exatamente uma `Section`. |
...
| **Post** | Subtipo concreto de `Artifact`. Conteúdo em texto prosa (MDX). Estruturalmente idêntico independente de onde está vinculado: pode estar vinculado a um `Source` (nota de curso, review de livro, anotação de cert) ou diretamente a uma Section `direct` (post de blog). **Rótulo de tipo derivado (contrato de UI)** — usado em kickers como o do "Em destaque", deriva da `Section` pai: Courses→"nota de curso", Books→"review", Certifications→"anotação", Blog→"blog". `Presentation` em Talks→"apresentação". Posts sob `Source` qualificam com o nome do `Source` (ex.: "nota de curso · AI Coding for Real Engineers"); blog e apresentação não. |
...
| **Now Learning** *(UI: "Agora estudando")* | **Projeção derivada** (read-model), não entidade persistida nem superfície de autoria. Conjunto de `Source`s e `Artifact`s `published` cujo status de estudo é `ongoing`, ordenado por atividade recente. Exibido na home. **Andamento é qualitativo, nunca quantificado:** não há percentual nem barra de progresso (decisão de redesenho 2026-06-12, alinha ADR-0019 ao protótipo). O card mostra um *detail* curto **opcional** autorado pelo autor (ex.: "módulo 3 de 5 · janela de contexto") e a **recência derivada** do `Artifact` `published` mais recente vinculado àquele `Source` (rótulo relativo "há N dias"). O único controle manual é o `study_status` (`ongoing`→`concluded`). |
