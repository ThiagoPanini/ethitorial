# ADR 0020 — Provedor de autenticação: better-auth (self-hosted)

- **Status:** Accepted
- **Data:** 2026-06-12
- **Decisores:** Thiago Panini (solo)
- **Relacionado:** [ADR-0007](0007-publicar-e-papel-de-user.md), [ADR-0011](0011-url-publica-do-publicador.md), [ADR-0017](0017-desenvolvimento-autonomo-afk.md), [docs/CONTEXT.md](../CONTEXT.md)
...
A stack já tem Postgres 17 (provisionado na sessão da issue #55) e um modelo `User` com `username` imutável (ADR-0011) e `role`. O escopo de auth da V1 é simples: e-mail/senha + sessão persistida. OAuth social fica para E0c (#61).
...
- `BETTER_AUTH_SECRET` gerado por máquina, setado via MCP (🟡), registrado em `docs/ai-ops/`.
...
O epistemix é um hub pessoal público com um único publicador admin. O volume de usuários autenticados (comentaristas/votantes) é baixo. Pagar por um SaaS de auth para este volume não faz sentido estratégico.
