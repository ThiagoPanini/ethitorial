...
- **[ADR-0013 — Substrato de planejamento operado por agentes (GitHub Projects)](../adr/0013-substrato-de-planejamento-operado-por-agentes.md)** — a decisão e o porquê: partição por altitude aplicada ao `ROADMAP.md` + issues + board, a máquina de estados concreta, a assimetria de autonomia, e o apêndice operacional de setup.

> **Nota de aplicação (variante concreta).** O corpo desta lição usa a máquina de **6 estados** (`Backlog`·`Todo`·`In progress`·`In review`·`Done`·`Blocked`) como ilustração genérica. O talkingpres adotou uma variante de **4 estados** (`Backlog`·`In progress`·`In review`·`Done`), com `Blocked` e `someday` como **labels**, porque o despacho é manual (do VS Code), não pull autônomo — o que colapsa `Todo` em `Backlog` e dispensa a coluna `Blocked`. E opera o board pelo **GitHub MCP server**, não por scripts. Os princípios (partição, assimetria, status efêmero, mínimo viável) valem idênticos; a contagem de estados e o motor de operação são escolhas de calibragem, exatamente como a [seção do loop mínimo](#comece-pelo-loop-mínimo-automatize-só-ao-sentir-dor) prevê.

> **Nota de evolução (mai/2026).** Após uso prático do desenho acima, o talkingpres migrou para uma calibragem mais leve: o **`ROADMAP.md` versionado** virou single source de plano *e* estado (markers `🚧`/`[x]`, sufixo `@human`/`@agent`), operado pela skill `solo-dev-assistant` em vez de um board ativo. A partição continua válida — mudou só o eixo (de altitude para tipo de dado: plano+estado no ROADMAP, discussão em issue opcional, código no PR). Decisão e tabela de revisão linha-a-linha no [ADR-0014](../adr/0014-roadmap-como-source-skill-solo-dev-assistant.md); o board GitHub Projects fica deferido, com trip-wires para reabrir. Esta lição segue válida como aula sobre a classe de design board-based — o talkingpres hoje é só uma calibragem diferente dela.
